...
class EggHeadInFridgePlayer(Player):
    ...
    def choose_move(self, board, board_point, uid) -> int:
        ...
        shorter_side = np.maximum(distance_start, distance_end)
        best_ones = np.flatnonzero(shorter_side[idxs] == np.max(shorter_side[idxs]))
